...
VkShaderModule ShaderManager::createShaderModuleFromByteCode(VkDevice &device, const std::string &shaderByteCode)
{
    // the shaderByteCode string is cast to a uint32_t, so it needs to be realloced in order to be aligned on 4 bytes.
    size_t alignment = 4;
...
VkShaderModule ShaderManager::createShaderModuleFromPath(VkDevice &device, const fs::path &entry)
{
