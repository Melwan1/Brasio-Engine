...
namespace brasio::io::files
{
    ...
    void OBJParser::dump(const fs::path &outputPath)
    {
        ...
        {
            ofs << "v " << vertex.at(0) << " " << vertex.at(1) << " " << vertex.at(2) << " "
                << vertex.at(3) << "\n";
        }
        for (const TextureCoordinatesType &textureCoordinates : _textureCoordinates)
        {
            ofs << "vt " << textureCoordinates.at(0) << " " << textureCoordinates.at(1) << " "
        ...
        {
            ofs << "vp " << parameterVertex.at(0) << " " << parameterVertex.at(1) << " "
                << parameterVertex.at(2) << "\n";
        }
    ...
    void OBJParser::_skipIgnoredLines()
    {
        ...
        while (_ifs.good()
               && (_nextLine.empty() || _nextLine.starts_with("#") || _nextLine.starts_with("o")
                   || _nextLine.starts_with("g") || _nextLine.starts_with("mtllib")
                   || _nextLine.starts_with("usemtl") || _nextLine.starts_with("s")))
        {
            _getNextLine();
        }
    }
    ...
    ElementIndexType OBJParser::_parseElementIndex(const std::string &elementIndexDefinition)
    {
        ...
        if (!elementIndexStream.eof())
        {
            BRASIO_LOG_ERROR("OBJ file (" + _objPath.string() + ") at faces section, line "
    ...
    void OBJParser::_error(const std::string &section, unsigned line, bool isIndex)
    {
        ...
        oss << "OBJ file (" << _objPath.string() << ") at " << section << " section, line " << line
            << " is lacking mandatory " << (isIndex ? "indices" : "coordinates");
