...
namespace brasio::renderer::vulkan::builders
{
    ...
    {
        _structureType = VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO;

        return withPrimitiveRestartEnable(VK_FALSE).withPrimitiveTopology(
            VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST);
    }

    InputAssemblyBuilder &InputAssemblyBuilder::withPrimitiveRestartEnable(const VkBool32 &enabled)
    {
    ...
    InputAssemblyBuilder &InputAssemblyBuilder::withConfig(const YAML::Node &config)
    {
        ...
        return withPrimitiveRestartEnable(config["primitive_restart"].as<bool>())
            .withPrimitiveTopology(
                primitiveTopologyMap.at(config["primitive_topology"].as<std::string>()));
    }
