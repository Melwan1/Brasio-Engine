    ...
    const std::string &extension)
{
    ...
    Logger::debug(std::cout, check_message_oss.str());
    int shaderCount =
        std::count_if(_shaderPaths.begin(), _shaderPaths.end(),
    ...
    Logger::trace(std::cout,
                  "Found " + std::to_string(shaderCount) + " shaders with the "
                      + extension + " extension in the graphics pipeline",
                  { "CREATE" });
    bool res = shaderCount == 1;
    if (!res)
    {
        Logger::error(std::cout,
                      "Should have exactly 1 shader with the " + extension
                          + " extension",
                      { "CREATE" });
    }
    return res;
