...
void SyncObjects::waitSemaphores(const std::vector<uint32_t> &indices)
{
    ...
    Logger::info(std::cout, "Nothing to be done for semaphores");
}
...
SyncObjects::toFenceVector(const std::vector<uint32_t> &indices) const
{
    ...
    // if no indices are provided, all fences are to be put in the fence vector
    if (indices.empty())
    {
...
bool SyncObjects::_checkSemaphoreBounds(uint32_t index) const
{
    if (index >= _semaphores.size())
    {
        ...
        oss << "Semaphore at index " << index
            << " is out of bounds for vector of size " << _semaphores.size();
        Logger::warning(std::cout, oss.str());
        return true;
    }
    return false;
}
