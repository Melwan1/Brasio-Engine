...
{
    RenderPass::RenderPass(const VkDevice &logicalDevice, const VkRenderPassCreateInfo &createInfo)
        : Handler("render pass", [logicalDevice](const VkRenderPass &renderPass) {
            vkDestroyRenderPass(logicalDevice, renderPass, nullptr);
        })
    {
        BRASIO_LOG_TRACE("Creating render pass", { "CREATE" });
        BRASIO_VULKAN_CHECK(vkCreateRenderPass(logicalDevice, &createInfo, nullptr, &getHandle()),
