...
{
    Semaphore::Semaphore(const VkDevice &logicalDevice, const VkSemaphoreCreateInfo &createInfo)
        : Handler("semaphore", [logicalDevice](const VkSemaphore &semaphore) {
            vkDestroySemaphore(logicalDevice, semaphore, nullptr);
        })
    {
        BRASIO_LOG_TRACE("Creating semaphore", { "CREATE" });
        BRASIO_VULKAN_CHECK(vkCreateSemaphore(logicalDevice, &createInfo, nullptr, &getHandle()),
