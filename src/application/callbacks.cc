...
namespace brasio::application
{
    ...
    void keyCallback(GLFWwindow *window, int key, int scancode, int action, int mods)
    {
        ...
        case GLFW_PRESS: {
            events::subevents::KeyboardPressEvent event =
                events::subevents::KeyboardPressEvent::create(key, scancode, mods);
            application->KeyboardEventEmitter::fire(event);
            break;
        }
        case GLFW_REPEAT: {
            events::subevents::KeyboardLongPressEvent event =
                events::subevents::KeyboardLongPressEvent::create(key, scancode, mods);
            application->KeyboardEventEmitter::fire(event);
            break;
        }
        case GLFW_RELEASE: {
            events::subevents::KeyboardReleaseEvent event =
                events::subevents::KeyboardReleaseEvent::create(key, scancode, mods);
            application->KeyboardEventEmitter::fire(event);
            break;
        }
        }
    ...
    void framebufferSizeCallback(GLFWwindow *window, int width, int height)
    {
        ...
        events::subevents::WindowResizeEvent event(false, static_cast<unsigned>(width),
