...
namespace brasio::mesh
{
    ...
    Plane::build(int horizontal_tessellation, int vertical_tessellation)
    {
        ...
        vertices.reserve((horizontal_tessellation + 1) * (vertical_tessellation + 1));
        indices.reserve(horizontal_tessellation * vertical_tessellation * 2
                        * 3); // three indices per triangle, 2 triangles per face

        for (int vertical_tessellation_index = 0;
             vertical_tessellation_index <= vertical_tessellation; vertical_tessellation_index++)
        {
                 ...
                 horizontal_tessellation_index++)
            {
                vertices.emplace_back(geometry::Vertex(
                    { static_cast<float>(horizontal_tessellation_index) / horizontal_tessellation
                          - 0.5f,
                      0.0f,
                      static_cast<float>(vertical_tessellation_index) / vertical_tessellation
                          - 0.5f }));
                if (horizontal_tessellation_index > 0 && vertical_tessellation_index > 0)
                {
                    indices.emplace_back(vertical_tessellation_index * (horizontal_tessellation + 1)
